...
document.getElementById('send-btn').addEventListener('click', async () => {
    ...
    await fetchAIResponse(sessionId, userInput);
});
...
async function fetchAIResponse(sessionId, userInput) {
    try {
        const prompt = 'you are a friendly ai';
        const url = `https://api.siputzx.my.id/api/ai/llama33?prompt=${encodeURIComponent(prompt)}&text=${encodeURIComponent(userInput)}`;
